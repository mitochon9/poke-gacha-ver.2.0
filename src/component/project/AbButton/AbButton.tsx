...
export const AbButton: React.FC<AbButtonProps> = ({ setScreenType }) => (
  <>
    <div className="flex gap-x-4">
      <div data-testid={`${baseId}-B`} className="mt-4">
        <CircleButton buttonType="B" onClick={() => setScreenType("top")} />
      </div>

      <div data-testid={`${baseId}-A`}>
        <CircleButton buttonType="A" onClick={() => setScreenType("lottery")} />
      </div>
    </div>
  </>
);
