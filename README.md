...
現在のところ大きく３つのフェーズに分けられています。

1. とりあえず作ってみた
2. バグが起きないように作ってみた
3. 保守性と可読性を意識して作ってみた

このドキュメント執筆時、エンジニアとして転職１ヶ月と少しなので保守性と可読性の面に関しては今後アップデートしていきたいと考えています。

## 目的

このアプリケーションの制作を通じて以下の力を身につけることを目的としています。
...
Array.from 関数で６４個の配列を作成し map 関数で表示しています。
...
今回はそのあたりを解消するべく実装してみました。以前のコードと並べて解説します。
...
今回は該当しない screenType では return させるようにすることで余計な記述が減らすことができました。
...
screenType が animation の時は return し、それ以外では トップ画面に遷移します。
...
ポケモンのデータを取得するために１〜１５１のランダムな番号を返します。
...
- 抽選で選ばれたデータを pokemonList に追加し localStorage に保存する
- localStorage に保存された pokemonList のデータを削除する

データの取得には SWR を使用しており、データが取得後に変化することはないので再検証を行わない useSWRImmutable を使用しています。
...
スクロール位置は currentHeight で管理しています。この値が ０〜ポケモン図鑑画面の高さ内に制御しないとボタンを押しても画面上の変化がないといったことが起きてしまいます。
